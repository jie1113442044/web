        ...
        <div class="screen">
            ...
            <el-card class="yxtj">
                <div flex="main:justify">
                    <div>给您推荐了 <span style="color:#4CBCFF">{{num}}</span> 个您可能需要的职位</div>
                </div>
            </el-card>
        </div>
        <div class="box-table2"  flex="dir:left">
            <div class="table-left"style="width:35%;height:100%">
            ...
            <div class="table-right" :class="state?'after-b':''" flex="dir:top">
                ...
                <div class="right-third">
                    <div class="map">
                <baidu-map class="bm-view" :center="{lng: tableDataa.mapLng, lat: tableDataa.mapLat}" :zoom="15":scroll-wheel-zoom="true">
                <bm-marker :position="{lng: tableDataa.mapLng, lat: tableDataa.mapLat}" :dragging="true" animation="BMAP_ANIMATION_BOUNCE" />
                </baidu-map>
                </div>
                </div>
            </div>
...
<script>
import{jobList,jobTuijian}from '../api/job'
import axios from 'axios'
import JobinformationVue from './Jobinformation.vue';
export default {
    data(){
        return{
            current:1,
    ...
    methods: {
        getNum(){
            this.tuijianData=localStorage.getItem("tuijian")
            this.tableData=JSON.parse(this.tuijianData)
            ...
            let res=[];
            for( var i=0;i<len;i++){
                tempjson[JSON.stringify(this.tableData[i])] = true;
            }
            let items=Object.keys(tempjson);
            for(var j=0 ;j< items.length ;j++){
                res.push(JSON.parse(items[j]))
            }
            this.tableData=res;
            console.log(res)
            for(var i=0;i<res.length;i++){
                    this.splitTemp=res[i].joblabel.split(",");
